...
interface DepthCarouselProps {
  ...
  className?: string;
  // UI screenshots carry small text, so they need a higher quality than
  // next/image's default of 75 to stay legible when scaled.
  quality?: number;
  // "contain" shows a whole screen uncropped — the right choice for app
  // screenshots. "cover" stays the default for photographic content.
  fit?: "cover" | "contain";
}
        ...
        {data.map((item, i) => (
          ...
          >
            <Image
              ...
              alt={item.alt || ""}
              fill
              // Asks for more than the CSS box so the browser's srcset pick
              // still has headroom on high-DPR screens and while the carousel
              // scales cards up.
              sizes={`${Math.round(cardWidth * 2)}px`}
              quality={quality}
